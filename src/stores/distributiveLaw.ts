...
import { pickRandom, shuffleArray, roundTo, type RandomSource } from '../utils/math'
...
function randSplit(total: number, dp: number, rng: RandomSource = Math.random): [number, number] {
  const part1 = roundTo(rng() * (total * 0.8 - total * 0.2) + total * 0.2, dp)
...
function generateExpand(
  ...
  if (rng() < trapRate) {
    ...
    const a = pickRandom([0.5, 2, 3, 10, 0.2, 5], rng)
    const target = pickRandom([10, 20, 50, 100, 1, 2, 5], rng)

    let b: number
    if (target > 2) {
      b = Math.floor(rng() * (target - 1)) + 1
    } else {
      b = rng() * (target - 0.1) + 0.1
    }
    b = roundTo(b, dp)
  ...
  const strategy = rng() < 0.5 ? 'special' : 'near_int'

  if (strategy === 'special') {
    // Special decimal expansion: 1.25×(80+8)
    const keys = Object.keys(SPECIAL_PAIRS).map(Number)
    const a = pickRandom(keys, rng)
    const pairs = SPECIAL_PAIRS[a]

    if (!pairs || pairs.length < 2) {
      return generateExpand(maxValue, dp, trapRate, rng)
    }

    // Pick two distinct indices
    const indices = (
      pairs.length === 2 ? [0, 1] : shuffleArray([...Array(pairs.length).keys()], rng).slice(0, 2)
    ) as [number, number]
    ...
    const a = pickRandom([0.5, 1.5, 2.5, 3.5, 6, 8, 0.6, 25, 2], rng)
...
function generateFactor(
  ...
  if (!pairs || pairs.length === 0) {
    return generateFactor(maxValue, dp, swapRate, rng)
  }

  const targetSum = pickRandom(pairs, rng)

  if (!targetSum || targetSum <= 2) {
    return generateFactor(maxValue, dp, swapRate, rng)
  }

  const [b, c] = randSplit(targetSum, dp, rng)
  ...
  if (op === '+') {
    ...
    const cVal = roundTo(rng() * 49 + 1, dp)
...
export function generateProblem(
  ...
  const isExpand = rng() < 0.5
  return isExpand
    ? generateExpand(maxValue, decimalPlaces, enableTrap ? 0.2 : 0, rng)
    : generateFactor(maxValue, decimalPlaces, enableSwap ? 0.2 : 0, rng)
}
